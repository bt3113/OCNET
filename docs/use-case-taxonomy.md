...
- **Merging** requires two approved Use Cases, moves Build links (duplicates collapse; order is renumbered with the primary first), sources and aliases; the old title becomes an `alternate` alias; the old slug redirects via `use_case_redirects`, and earlier redirects into the merged Use Case are re-pointed so chains keep resolving. Nothing is deleted.
- **Redirects** also cover retired broad areas (to a category page) and the old `/builds/xai-*` pages (to the matching Use Case).

In connected mode these run through admin-only `security definer` functions (`map_use_case_proposal`, `approve_use_case_proposal`, `reject_use_case_proposal`, `merge_use_cases`, `archive_use_case`) that write `audit_events`. Direct writes to taxonomy tables are admin-only under RLS; providers can only insert or withdraw their own pending proposal for a Build they own, with the Build's own creator, platform provenance (`community supplied`) and a current timestamp; moderation functions write their own provenance.
