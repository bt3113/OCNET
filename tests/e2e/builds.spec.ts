...
test("build discovery, graph, alternatives and collection persistence", async ({
  ...
  await page.locator(".build-card h3 a").click();
  ...
  await page.getByRole("tab", { name: "Stack & architecture" }).click();
...
test("publish wizard validates, autosaves and publishes a usable blueprint", async ({
  ...
  await page
    .getByRole("button", { name: "Start manually", exact: true })
    .click();
  await page
    .getByLabel("Build name", { exact: true })
    .fill("QA support blueprint");
  await page
    .getByLabel("Short outcome / tagline")
    .fill("Answer support questions from approved knowledge.");
  await page
  ...
  for (let i = 0; i < 4; i++)
    await page.getByRole("button", { name: "Continue", exact: true }).click();
  await page
    .getByRole("button", { name: "Publish demo build", exact: true })
    .click();
  await expect(
    page.getByText(/Confirm your rights to publish this work/),
  ).toBeVisible();
