...
describe("Build graph RLS on actual PostgreSQL", () => {
  ...
  it("claims are claimant/admin only and cannot be self-approved", async () => {
    await identity(owner);
    await db.exec(
      `insert into public.provider_claims(id,name,"providerId","ownerId",evidence,domain,"contactEmail") values('claim','Claim','vendor','${owner}','Company representation evidence provided for review','vendor.example','owner@vendor.example')`,
    );
    await identity(other);
    expect(
